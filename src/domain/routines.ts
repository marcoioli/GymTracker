...
export function normalizeRoutineExercise(
	exercise: LegacyRoutineExercise,
): RoutineExercise {
	const setReferences = normalizeExerciseSetReferences(
	...
	return {
		...
		muscle: isMuscleGroup(exercise.muscle)
			? exercise.muscle
...
export function createExerciseSetReference(
	seed?: Partial<Omit<RoutineExerciseSetReference, "id">>,
): RoutineExerciseSetReference {
	return {
		id: crypto.randomUUID(),
		weightTarget: seed?.weightTarget ?? "",
...
function normalizeExerciseSetReferences(
	...
	const legacyReferences = Array.isArray(value)
		? value
				.filter(
					(entry): entry is Partial<RoutineExerciseSetReference> =>
						Boolean(entry) && typeof entry === "object",
				)
				.map((entry) =>
					createExerciseSetReference({
						weightTarget:
							typeof entry.weightTarget === "string" ? entry.weightTarget : "",
						repsTarget:
							typeof entry.repsTarget === "string" ? entry.repsTarget : "",
						rirTarget:
							typeof entry.rirTarget === "string" ? entry.rirTarget : "",
					}),
				)
		: [];
...
export function createRoutineWeek(index: number, dayCount = 1): RoutineWeek {
	return {
		...
		days: Array.from({ length: dayCount }, (_, dayIndex) =>
			createRoutineDay(dayIndex),
		),
	};
}

export function cloneRoutineWeekStructure(
	...
	return {
		...
		days: normalizedWeek.days.map((day) => ({
			...
			exercises: day.exercises.map((exercise) => {
				...
				const setReferences =
					normalizedExercise.setReferences ??
					normalizeExerciseSetReferences(
						undefined,
						normalizedExercise.targetSets,
						normalizedExercise.targetRir,
					);
				...
				return {
					...
					setReferences: setReferences.map((reference) =>
						createExerciseSetReference({
							weightTarget: reference.weightTarget,
...
export function getWeeklyMuscleVolumeByWeek(
	routine: Routine,
	weekIndex: number,
): WeeklyMuscleVolume {
	...
	for (const day of week.days) {
		for (const exercise of day.exercises) {
			weeklyVolume[normalizeRoutineExercise(exercise).muscle] +=
				exercise.targetSets;
		}
	}
...
export function getCurrentWeeklyMuscleVolume(
	routine: Routine,
): WeeklyMuscleVolume {
	return getWeeklyMuscleVolumeByWeek(
		normalizeRoutineRecord(routine),
		resolveRoutineProgress(routine).currentWeekIndex,
	);
}

export function listRoutineDaySelections(
	routine: Routine,
): RoutineDaySelection[] {
...
export function resolveRoutineProgress(routine: Routine): RoutineProgress {
	...
	const currentWeekIndex = Math.min(
		Math.max(routine.progress.currentWeekIndex, 0),
		routine.weeks.length - 1,
	);
	const hasMatchingCompletedDay = listRoutineDaySelections(routine).some(
		(selection) => selection.day.id === routine.progress.lastCompletedDayId,
	);

	return {
		currentWeekIndex,
		lastCompletedDayId: hasMatchingCompletedDay
			? routine.progress.lastCompletedDayId
			: null,
		lastCompletedAt: hasMatchingCompletedDay
			? routine.progress.lastCompletedAt
			: null,
	};
}

export function getSuggestedRoutineDay(
	routine: Routine,
): RoutineDaySelection | null {
	const selections = listRoutineDaySelections(routine).filter((selection) =>
		isTrainableRoutineDay(selection.day),
	);
	...
	const preferredWeekIndex = Math.min(
		Math.max(progress.currentWeekIndex, 0),
		routine.weeks.length - 1,
	);
	...
	if (!progress.lastCompletedDayId) {
		return (
			selections.find(
				(selection) => selection.weekIndex === preferredWeekIndex,
			) ?? null
		);
	}

	const lastCompleted = selections.find(
		(selection) => selection.day.id === progress.lastCompletedDayId,
	);
	...
	const nextDayInSameWeek = selections.find(
		(selection) =>
			selection.weekIndex === lastCompleted.weekIndex &&
			selection.dayIndex > lastCompleted.dayIndex,
	);
	...
	return (
		selections.find((selection) => selection.weekIndex === nextWeekIndex) ??
		selections[0]
	);
}

export function listTrainableRoutineDaySelections(
	routine: Routine,
): RoutineDaySelection[] {
	return listRoutineDaySelections(routine).filter((selection) =>
		isTrainableRoutineDay(selection.day),
	);
}
...
export function advanceRoutineProgress(
	...
	const nextWeekIndex =
		week && selection.dayIndex === week.days.length - 1
			? (selection.weekIndex + 1) % routine.weeks.length
			: selection.weekIndex;
...
export function resolveActiveRoutine(
	routines: Routine[],
	activeRoutineId: string | null,
): ActiveRoutineResolution {
	const validRoutineIds = new Set(routines.map((routine) => routine.id));
	const validPointer =
		activeRoutineId && validRoutineIds.has(activeRoutineId)
			? activeRoutineId
			: null;
	const activeCandidates = routines
		.filter((routine) => routine.status === "active")
		.sort((left, right) => right.updatedAt.localeCompare(left.updatedAt));
	const chosenRoutine = validPointer
		? (routines.find((routine) => routine.id === validPointer) ??
			activeCandidates[0] ??
			null)
	...
	return {
		repairedState:
			Boolean(activeRoutineId && !validPointer) ||
			activeCandidates.length > 1 ||
			(chosenRoutine ? chosenRoutine.status !== "active" : false),
