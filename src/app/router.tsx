...
const HistorySessionDetailPage = lazy(async () => ({
	default: (await import("../features/history/HistorySessionDetailPage"))
		.HistorySessionDetailPage,
}));
const AnalyticsPage = lazy(async () => ({
	default: (await import("../features/analytics/AnalyticsPage")).AnalyticsPage,
}));
...
function RootLayout() {
	...
	const isWorkoutsHub =
		location.pathname === "/" || location.pathname === "/routines";
		...
		<div className="app-shell app-shell--mobile">
			...
			<header className="app-brand-header" aria-label="Treino">
				<NavLink
					...
					className="app-brand-header__link"
					to="/"
				>
					<img
						alt=""
						aria-hidden="true"
						className="app-brand-header__mark"
						src="/icons/logoTreinoPNG.png"
					/>
			...
			{isWorkoutsHub ? (
				<header
					className="workouts-shell-header"
					aria-label="Encabezado de workouts"
				>
					<nav
						aria-label="Cambiar vista de workouts"
						className="workouts-segmented-nav"
					>
						<NavLink
							className={({ isActive }) =>
								`workouts-segmented-nav__link${isActive ? " active" : ""}`
							}
							end
							to="/"
						>
					...
					{!isTrackerRoute ? (
						<button
							aria-label={fabCopy.label}
							className="workouts-primary-cta"
							type="button"
			...
			<main
				className="app-main app-main--mobile"
				id="main-content"
				ref={mainRef}
				tabIndex={-1}
			>
	...
	return (
		...
		>
			<StatusBanner tone="info">
				La app está abriendo el módulo sin romper el flujo mobile principal.
			</StatusBanner>
		</PageSection>
	);
}
