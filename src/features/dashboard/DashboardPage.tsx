...
export function DashboardPage() {
	...
	const suggestedDay = activeRoutine
		? getSuggestedRoutineDay(activeRoutine)
		: null;
	...
	return (
		<>
			<PageSection
				title={<span className="sr-only">Tracker</span>}
				titleId="tracker-title"
			>
				{sessionSavedMessage ? (
				...
				{activeRoutine && suggestedDay ? (
					<Card as="article" className="tracker-focus-card" variant="highlight">
						<div className="tracker-focus-card__header">
							...
							<span className="tracker-focus-card__badge">
								{suggestedDay.day.exercises.length} ejercicios
							</span>
						</div>
						...
						<div className="tracker-focus-card__stats">
							<TrackerMiniStat
								label="Series"
								value={`${suggestedDay.day.exercises.reduce((total, exercise) => total + exercise.targetSets, 0)}`}
							/>
							<TrackerMiniStat
								label="Última vez"
								value={formatLastCompleted(
									activeRoutine.progress.lastCompletedAt,
								)}
							/>
						</div>
				...
				{sessions.length === 0 ? (
					...
					<div
						className="tracker-history-stack"
						role="list"
						aria-label="Historial reciente de entrenamientos"
					>
			...
			{activeRoutine && isModalOpen ? (
				<ConfirmWorkoutDayModal
					...
					onConfirm={({ weekIndex, dayId }) => {
						...
						navigate(
							`/session/${activeRoutine.id}/${weekIndex}/${dayId}?startedAt=${encodeURIComponent(new Date().toISOString())}`,
						);
					}}
				/>
			) : null}
		...
		>
			<div className="tracker-history-card__header">
				<div>
					...
					<p className="routine-summary">
						{session.routineName ?? "Tracked Workout"}
					</p>
				</div>
			...
			<p className="tracker-history-card__preview">
				{formatExercisePreview(session)}
			</p>

			<div
				className="tracker-history-card__metrics"
